function [K,lim_cov,beta_0,beta_n,t_BrAC,e_BrAC,t]=delta_method_quadratic_simulations(q_0,sigma_1,sigma_2,C,D,E,F,dim,S,M,m,Gamma,q_est)
...
B=@(q2) q2*F;
pol_1=@(u) sqrt(3)*u;
pol_2=@(u) sqrt(80)*(u^2-0.75*u);
mu=@(u) -0.2*u*(u-1);
f_mu=@(s)integral(@(u)arrayfun(@(U)C*expm(A(q_0(1))*(s-U))*B(q_0(2))*mu(U),u),0,s);
psi{1}=@(q1,q2,s)integral(@(u)arrayfun(@(U)C*expm(A(q1)*(s-U))*B(q2)*pol_1(U),u),0,s);
psi{2}=@(q1,q2,s)integral(@(u)arrayfun(@(U)C*expm(A(q1)*(s-U))*B(q2)*pol_2(U),u),0,s);
psi_=@(q1,q2,s)[psi{1}(q1,q2,s);psi{2}(q1,q2,s)];
prl_psi_1{1}=@(q1,q2,s)integral(@(u)arrayfun(@(U)C*der_matrix_exp(-D,-E,q1,s-U,dim+1)*B(q2)*pol_1(U),u),0,s);
prl_psi_1{2}=@(q1,q2,s)integral(@(u)arrayfun(@(U)C*der_matrix_exp(-D,-E,q1,s-U,dim+1)*B(q2)*pol_2(U),u),0,s);
prl_psi_2{1}=@(s,q1,q2)integral(@(u)arrayfun(@(U)(1/q2)*C*expm(A(q1)*(s-U))*B(q2)*pol_1(U),u),0,s);
prl_psi_2{2}=@(q1,q2,s)integral(@(u)arrayfun(@(U)(1/q2)*C*expm(A(q1)*(s-U))*B(q2)*pol_2(U),u),0,s);
...
lim_cov=sigma_1^2*transpose(K)*inv(Gamma)*K;
...
for i=1:M
    est_BrAC{i}=@(u) beta_n(i,1)*pol_1(u)+beta_n(i,2)*pol_2(u);
